...
user.name = 'Daniel Kanz' # set to your user name
...
## Exercise 3 -- Wiederholungsaufgaben

Betrachten Sie folgende Variablen und wie sie Zusammenhaengen:
...
```
a) Sie sind an dem kausalen Effekt von `x1` auf `y` interessiert. Bei einer einfachen linearen Regression von `x1` auf `y` haben Sie bereits einen positiven Effekt gefunden. Was passiert mit dem Schaetzer wenn Sie `x2` der Regression hinzufuegen. Wird er kleiner oder groesser?
...
b) Und wenn Sie zuerst einen negativen Effekt gefunden haetten?
...
1) Ein signifikanter Schaetzer fuer beta von -2.5 spricht fuer einen groesseren Zusammenhang zwischen erklaerender und abhaengiger Variable als ein signifikanter Schaetzer von -0.8 in der gleichen Regression.

2) Heterogene Effekte koennen durch eine Aufteilung der Stichprobe oder durch Interaktionsterme in der Regression aufgefangen werden.

3) Mehr Kontrollvaribalen erhoehen die Wahrscheinlichkeit den wahren/kausalen Effekt von erklaerender auf abhaengige Variable zu schaetzen.

4) Ist die abhaengige Variable eine Dummyvariable, kann der Schaetzer als Wahrscheinlichkeit interpretiert werden. 
...
d) Welche Variable sollten Sie zwingend mit in die Regression aufnehmen, wenn Sie einen kausalen Effekt zwischen x1 und y messen wollen? Ist es x2, x3, oder erhalten Sie einen kausalen Effekt nur, wenn Sie die Variablen in Kombination aufnehmen? Fuehren Sie zur Beantwortung der Frage unten stehenden Code aus.

```{r "3_d"}
set.seed(2019)
n <- 10000
beta1 <- 5 # wahrer kausaler effekt
x2 <- rnorm(n,0,12)
x1 <- rnorm(n,0,1)
x3 <- rnorm(n,3,5)-x1*1.7
y <- beta1*x1 + x2 + x3 + rnorm(n,0,1)

# Regression mit allen Kontrollvariablen
reg1 <- lm(y~x1+x2+x3)
# Regression ohne Kontrollvariablen
reg2 <- lm(y~x1)
# Regression mit x2 als Kontrollvariable
reg3 <- lm(y~x1+x2)
# Regression mit x3 als Kontrollvariable
reg4 <- lm(y~x1+x3)
# Zeige Ergbenisse nebeneinander
library(stargazer)
stargazer(reg1, reg2, reg3, reg4, type="text", digits=3, keep.stat=c("n"))
```
